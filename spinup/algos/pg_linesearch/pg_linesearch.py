...
import gym
import time
import spinup.algos.pg_linesearch.core as core
...
Policy Gradient With linesearch
Every gradient step, penalty from env is checked before policy params are updated.
Every polocy params updates happen only if they are safer (as defined by the env) than the previous policy
...
def pg_linesearch(env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=dict(), seed=0, 
    ...
    pi_name = "pi"
    scope_variable = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=pi_name)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', type=str, default='pg_linesearch')
    ...
    pg_linesearch(lambda : gym.make(args.env), actor_critic=core.mlp_actor_critic,
