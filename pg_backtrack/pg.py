...
class PG(object):
	...
	def build_policy_network_op(self, scope = "policy_network"):
		...
		if self.discrete:
			action_logits = build_mlp(mlp_input, output_size, scope, n_layers, size)
			...
			action_means = build_mlp(mlp_input, output_size, scope, n_layers, size)
			# log_std spreads over R which is better for training
			...
			# use re-parametrization as indicated in piazza: to enable gradient back-propagation
			self.sampled_action = action_means + tf.exp(log_std) * tf.random_normal(shape=tf.shape(action_means))
	...
	def add_loss_op(self):
		...
		######################################################
		# - bcz we actually want to maximize
	...
	def add_baseline_op(self, scope = "baseline"):
		...
		self.baseline = build_mlp(mlp_input, output_size, scope, n_layers, size)
